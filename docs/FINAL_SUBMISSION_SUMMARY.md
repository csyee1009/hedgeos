...
2. **AI Intent Compilation:** The configured Gemini model maps the prompt to a structured `TypedRiskIntent` draft.
...
- Configured Gemini intent parsing, with the active provider model reported by the runtime status endpoint.
